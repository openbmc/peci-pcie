                                  ...
                                  const int& dev, std::string& generationInUse)
{
    ...
    constexpr int nthStair = 3;
    constexpr int offsetLength = 2;
    uint32_t linkStatus;

    error = getCapReading(clientAddr, bus, dev, nthStair, linkStatus, pcieCapID,
                          peci_pcie::linkStatusOffset, offsetLength);
    if (error != resCode::resOk)
    {
        return error;
    }
