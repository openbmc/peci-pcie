...
static std::tuple<std::string, resCode>
    getStringFromPCIeConfig(const int& clientAddr, const int& bus,
                            const int& dev, const int& func, const int& offset,
                            const int& size)
{
    ...
    if (getDataFromPCIeConfig(clientAddr, bus, dev, func, offset, size, data) !=
        resCode::resOk)
    {
        return std::make_tuple(std::string(), resCode::resErr);
    }
    ...
    dataStream << "0x" << std::hex << std::setfill('0') << std::setw(size * 2)
               << data;
    return std::make_tuple(dataStream.str(), resCode::resOk);
}

static std::tuple<std::string, resCode>
    getVendorName(const int& clientAddr, const int& bus, const int& dev)
{
    ...
    return std::make_tuple(
        pciVendors.try_emplace(vendorID, otherVendor).first->second,
        resCode::resOk);
}

static std::tuple<std::string, resCode> getDeviceClass(const int& clientAddr,
                                                       const int& bus,
                                                       const int& dev,
                                                       const int& func)
{
    ...
    return std::make_tuple(
        pciDeviceClasses.try_emplace(baseClass, otherClass).first->second,
        resCode::resOk);
}

static std::tuple<bool, resCode> isMultiFunction(const int& clientAddr,
                                                 const int& bus, const int& dev)
{
    ...
    {
        return std::make_tuple(false, resCode::resErr);
    }
    // Check if it's a multifunction device
    if (headerType & multiFuncBit)
    {
        return std::make_tuple(true, resCode::resOk);
    }
    return std::make_tuple(false, resCode::resOk);
}

static std::tuple<bool, resCode> pcieFunctionExists(const int& clientAddr,
                                                    const int& bus,
                                                    const int& dev,
                                                    const int& func)
{
...
static std::tuple<bool, resCode>
    pcieDeviceExists(const int& clientAddr, const int& bus, const int& dev)
{
                                         ...
                                         const int& dev, const int& func)
{
    ...
    auto [res, error] = getDeviceClass(clientAddr, bus, dev, func);
                                       ...
                                       const int& dev)
{
    ...
    auto [manuf, error] = getVendorName(clientAddr, bus, dev);
    if (error != resCode::resOk)
    {
        return error;
    }
    ...
    bool multiFunc;
    std::tie(multiFunc, error) = isMultiFunction(clientAddr, bus, dev);
    if (error != resCode::resOk)
    {
        return error;
    }
                                ...
                                const int& dev)
{
    ...
    {
        auto [res, error] = pcieFunctionExists(clientAddr, bus, dev, func);
        if (error != resCode::resOk)
        {
            return error;
        }
...
static resCode probePCIeDevice(boost::asio::io_service& io,
                               ...
                               int dev)
{
    auto [res, error] = pcieDeviceExists(cpuInfo[cpu].addr, bus, dev);
    if (error != resCode::resOk)
    {
        return error;
    }
                           ...
                           int dev)
{
        ...
        resCode::resOk)
    {
        ...
        if (pcieRetryCount < 5)
        {
            std::cout << "There was an error while probing CPU " << cpu
                      << " Bus " << bus << " Device " << dev << "\n";
            static boost::asio::steady_timer pcieRetry(io);
            constexpr const int pcieRetryTime = 1;
            pcieRetry.expires_after(std::chrono::seconds(pcieRetryTime));
            pcieRetry.async_wait([&io, &objServer, &cpuInfo, cpu, bus,
                                  dev](const boost::system::error_code& ec) {
                if (ec)
                {
                    ...
                    {
                        std::cerr << "PECI PCIe Retry async_wait failed " << ec;
                    }
                    return;
                }
